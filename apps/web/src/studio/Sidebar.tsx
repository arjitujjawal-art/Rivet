...
export function Sidebar({ scenes, activeId, onSelect, mode, modes, onModeSelect }: Props) {
  const [awsInfo, setAwsInfo] = useState<{
    ...
    status: 'online',
    cloud_mode: 'aws_native',
    s3: { bucket: 'rivet-campaigns' },
    bedrock: { model_id: 'Claude 3.5 Sonnet' },
  })
  ...
  return (
    <aside className="hidden w-[238px] shrink-0 flex-col bg-[#09090b] md:flex">
      ...
      <div className="mt-auto border-t border-white/[0.06] p-3">
        ...
        <div className="mt-3 flex items-center gap-2 rounded-lg border border-white/[0.06] bg-white/[0.02] p-2">
          <span className="h-2 w-2 shrink-0 rounded-full bg-success shadow-[0_0_8px_rgba(52,211,153,0.6)] animate-pulse" />
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between">
              <p className="truncate text-[9px] font-medium text-white">AWS Cloud Native</p>
              <span className="rounded bg-amber-500/10 px-1 py-0.2 font-mono text-[7px] font-semibold text-amber-400">
                BEDROCK + S3
              </span>
            </div>
            <p className="mt-0.5 truncate font-mono text-[8px] text-faint">
              s3://{awsInfo.s3?.bucket || 'rivet-campaigns'}
            </p>
          </div>
        </div>
      </div>
