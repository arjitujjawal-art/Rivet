...
function CheckRow({ check }: { check: Check }) {
  ...
  return (
    <div
      className={`group flex items-center gap-3 border-b border-white/[0.045] py-2.5 last:border-0 ${
        failed ? 'rounded-lg bg-danger/[0.06] px-2' : ''
      }`}
    >
      <span
        className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${
          failed
            ? 'bg-danger/20 text-danger ring-2 ring-danger/30'
            : advisory
      ...
      <span className="min-w-0 flex-1">
        <span
          className={`block truncate text-[11px] font-medium ${
            failed ? 'font-semibold text-danger' : 'text-muted'
          }`}
        >
          {check.metric}
        </span>
        <span className="mt-0.5 block truncate font-mono text-[8px] text-faint">
          {check.id} · Req: {check.threshold}
        </span>
      </span>
      <div className="text-right">
        <span
          className={`font-mono text-[10px] ${
            failed
              ? 'rounded bg-danger/20 px-1.5 py-0.5 font-bold uppercase text-danger'
              : 'text-foreground'
          }`}
        >
          {check.observed}
        ...
        {failed && (
          <span className="block font-mono text-[7px] uppercase tracking-wider text-danger/80">
            Export Gated
          </span>
        )}
      </div>
        ...
        <section className="mt-5 rounded-[14px] border border-white/[0.06] bg-white/[0.025]">
          ...
          {receiptOpen && (
            <div className="border-t border-white/[0.055] px-3 py-2">
              ...
              <ReceiptRow label="AWS S3 Ledger" value={`s3://rivet-campaigns/${RECEIPT.projectId.slice(0, 8)}/receipt.json`} />
              <ReceiptRow label="Audit Result" value={verified ? "100% PASSED (EXPORT CERTIFIED)" : "FAILED (A01 - EXPORT GATED)"} />
              <ReceiptRow label="Content digest" value={RECEIPT.contentDigest} />
              <ReceiptRow label="Seed" value={String(scene.seed)} />
              <ReceiptRow label="Product SHA" value={RECEIPT.productSha} />
              <ReceiptRow label="Logo SHA" value={RECEIPT.logoSha} />
              <ReceiptRow label="Compute Device" value={RECEIPT.device} />
            </div>
          )}
        </section>
      </div>
